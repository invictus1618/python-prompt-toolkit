...
if TYPE_CHECKING:
    from .key_processor import KeyPressEvent, KeyPress
        ...
        if isinstance(filter, Never):
            ...
            def decorator(func: T) -> T:
                if isinstance(func, Binding):
                    ...
                    self.bindings.append(
                        Binding(keys, func, filter=filter, eager=eager,
...
class _MergedKeyBindings(_Proxy):
    ...
    :param registries: List of :class:`.KeyBindings` objects.
    """
    def __init__(self, registries: List[KeyBindingsBase]) -> None:
        _Proxy.__init__(self)
...
def merge_key_bindings(bindings) -> _MergedKeyBindings:
    """
